...
<div class="page-content">
    ...
    <form method="post" action="${formAction}" class="create-vacancy-form" accept-charset="UTF-8">
        <input class="create-form-input secondary" type="text" name="${vacancyNameParam}" value="${vacancy.vacancyName}"
               placeholder="${vacancyName}" pattern=".{1,30}" title="${nameTitle}" required/>
        <input class="create-form-input secondary" type="text" name="${companyNameParam}" value="${vacancy.companyName}"
               placeholder="${companyName}" pattern=".{1,30}" title="${nameTitle}" required/>
        <textarea class="create-form-input secondary create-form-textarea" name="${vacancyDescriptionParam}"
                  placeholder="${vacancyDescription}" required><c:out value="${vacancy.description}"/></textarea>
        <textarea class="create-form-input secondary create-form-textarea" name="${vacancySkillsDescriptionParam}"
                  placeholder="${vacancySkillsDescription}" required><c:out
                value="${vacancy.skillsDescription}"/></textarea>

        <input class="common-button" type="submit" value="${updateVacancyButton}">
    </form>
</div>
