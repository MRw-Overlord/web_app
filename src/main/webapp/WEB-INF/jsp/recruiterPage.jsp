    ...
    <div class="admin-content">
        <form method="post" accept-charset="UTF-8">
            <select class="description" name="recruiterListType" onchange="this.form.submit();">
                <option value="vacancies" ${recruiterListType == "vacancies" ? 'selected="selected"' : ''}><fmt:message
                        key="page.admin.option.vacancies"/></option>
                <option value="users" ${recruiterListType == "users" ? 'selected="selected"' : ''}><fmt:message
                        key="page.admin.option.users"/></option>
            </select>
        </form>
        <c:choose>
